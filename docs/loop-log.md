...
| — | 2026-08-10 | shipped | Reactive triage, not a scored cycle: documented and suppressed the CodeQL "critical" code-injection alert that PR #36 surfaced on `/eval`'s `Function` call — real per the scanner, already covered by `SECURITY.md`'s stated threat model, no behavior changed. | [#37](https://github.com/reagent-systems/clientside-containers/pull/37) | — | Per §7's reading of automated-tool signals, not the survey→ideate→score flow. Issue #15 (12/20) is still the highest-scoring open, unbuilt candidate for the next scheduled cycle. |
