...
| 2 | 2026-08-09 | shipped | A container's terminal (App bottle, Mini OS) no longer shows garbled leftover characters from an ANSI escape sequence — the parser now consumes the whole sequence instead of cutting it short after two bytes. | — | 17/20 | Found while surveying `lib/serial-terminal.ts` for test coverage, not from an open issue. Runners-up #14 (13/20) and #15 (12/20) are still open and still unbuilt; no new candidate this cycle scored ≥ 12, so no new issue was filed. |
